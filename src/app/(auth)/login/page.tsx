...
const LoginPage: React.FC = () => {
    ...
    const handleGoogleLogin = async () => {
        ...
        try {
            await googleSignIn(); // This handles the Google auth and profile lookup in AuthContext
            setMessage('Login successful via Google! Redirecting to dashboard...');
            router.push('/dashboard/user'); // Redirect to user dashboard (AuthContext will handle specific role)
        } catch (error) {
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-blue-100 to-indigo-100 p-4 flex flex-col items-center justify-center font-sans">
            <div className="w-full max-w-md bg-white rounded-xl shadow-2xl p-8 space-y-6">
                <h2 className="text-4xl font-bold text-center text-indigo-800">Login to Your Account</h2>
                ...
                <div className="space-y-4">
                    ...
                    >
                        ...
                        {loadingAuth ? 'Logging In...' : 'Sign In with Google'}
                    </Button>
                </div>
